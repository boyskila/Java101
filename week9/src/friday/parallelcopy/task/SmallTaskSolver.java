...
public class SmallTaskSolver implements Runnable {
	private DirectoryInfoStorage<FileInfo> dirInf;

	public SmallTaskSolver(DirectoryInfoStorage<FileInfo> dirInf) {
		this.dirInf = dirInf;
	}
	...
	public void run() {
		...
		Iterator<FileInfo> it = dirInf.smallerTasksIterator();
		while (it.hasNext()) {
			FileInfo fi = it.next();
			it.remove();
			try {
